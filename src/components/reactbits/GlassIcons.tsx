...
const GlassIcons: React.FC<GlassIconsProps> = ({ items, className, colorful = true }) => {
  const getBackgroundStyle = (color: string): React.CSSProperties => {
    ...
    return { background: 'linear-gradient(hsl(210, 80%, 45%), hsl(210, 80%, 35%))' };
  };
  ...
  return (
    <div className={`grid gap-[5em] grid-cols-2 md:grid-cols-3 mx-auto py-[3em] overflow-visible ${className || ''}`}>
      {items.map((item, index) => {
        ...
        const wrapperClass = `relative bg-transparent outline-none border-none cursor-pointer w-[4.5em] h-[4.5em] [perspective:24em] [transform-style:preserve-3d] [-webkit-tap-highlight-color:transparent] group ${
          item.customClass || ''
        }`;
